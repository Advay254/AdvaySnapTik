...
const CACHE = 'ast-v3';                          // bump version = old cache wiped
...
self.addEventListener('fetch', e => {
  ...
  // 1. Never intercept API calls — always live
  if (url.pathname.startsWith('/api/')) return;

  // 2. Never intercept external origins (fonts, ad networks, CDN)
  if (url.origin !== self.location.origin) return;

  // 3. HTML / navigation → NETWORK FIRST
  //    Always loads fresh HTML so server-injected ads always appear.
  ...
  // 4. Static assets (JS/CSS/images/audio) → STALE WHILE REVALIDATE
  //    Serve cached instantly, fetch fresh in background for next load.
  e.respondWith(
...
// ── Share target ──────────────────────────────────────────────────────────────
self.addEventListener('fetch', e => {
  if (new URL(e.request.url).pathname !== '/share-target') return;
  e.respondWith((async () => {
    const u = new URL(e.request.url);
    const su = u.searchParams.get('url') || '';
    const st = u.searchParams.get('text') || '';
